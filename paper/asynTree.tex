...
\end{lstlisting}
\end{minipage}
\caption{JSON representation of a simple command list being sent to an embedded motion controller. The top level container is an array which preserves order. Arguments here are named, but could also be positional. JSON text copied from file \textit{two.json} in the Springleik project repository on GitHub \cite{GitHubRepo}.}
\label{simpleListing}
\end{figure}
...
\end{lstlisting}
\end{minipage}
\caption{JSON representation of a composite command tree with conditional and repeating nodes. The top level container is a dictionary and a command node which does not preserve order. Subordinate node lists are contained in arrays which do preserve order. JSON text copied from file \textit{three.json} in the Springleik project repository on GitHub \cite{GitHubRepo}.}
\label{treeListing}
\end{figure}
...
\end{lstlisting}
\end{minipage}
\caption{Python code snippet from function \textit{figureFactory} in file \textit{host.py} in the Springleik project repository on GitHub \cite{GitHubRepo}. Shown are the steps to instantiate and compose the command tree serialized in figure \ref{treeListing}.}
\label{figureFactory}
\end{figure}
...
Reference files should be under version control, ideally alongside the implementation source code. Test files can also be versioned, especially if they contain additional content not in the reference files. This style of testing encourages the use of large scale bulk tests, which may seem unfamiliar to those accustomed to long lists of fine-grained assertions. In either case the workflow is the same: find all the deltas, rebaseline those that are expected, and repair those that aren't. One might ask where the refererence files come from. Experience shows that a variety of sources can be used including outputs from previous versions, synthetic data created programmaticaly, hand typed data, or hybrid combinations of these. The content of this section has been published previously in reference \cite{asymmetricRules}.

\section{Execution model}
Much of the work described here has been done on targets which support a real-time operating system (RTOS). Since the Springleik command tree carries along with it all of the arguments and parameters needed for execution, all that's needed to initiate a task thread to execute the tree is a pointer or reference to the root node. On exit the pointer or reference will identify a tree now having updated state information in its attributes, along with any instrumentation and data nodes appended during execution. Having a unique pointer or reference allows the developer to think clearly about the life cycle of each command tree. By limiting access to hardware and global data to specific commands or node classes, it becomes straightforward to implement synchronization primitives to maintain thread safety between command trees. Springleik command trees easily support cooperative multitasking by inserting appropriate thread waits. While one could introduce a node class to do this, it's easier to put waits into branch node execute methods to relinquish control between calls to subordinate nodes.

\section{Reuse and granularity}
One may well wonder how much of this can be reused across multiple projects. Our experience has shown that command nodes are clearly domain specific and can only be reused for very similar applications, while control nodes are not domain specific and can be reused freely. The resulting plethora of small languages can be justified by our need to maintain a small footprint by optimizing the command tree language for each application. A related question is how to set the granularity of command and control nodes. We propose starting with one command node class for each monitor command on the target, and one control node class for each branching or gating operation. If math and logic expressions are to be supported, start with one math or logic operation per control node. As development progresses common patterns will soon become apparent, which then become candidates for integration into more functional node classes. These are design decisions which don't have to be decided up front.

\section{Toy model}

\begin{figure}
\includegraphics[width=\textwidth]{toyModel}
\caption{Three axis motion control simulation implemented in a target process. A separate host process implements an abstract syntax tree (AST) interpreter using Springleik techniques. Source code is posted in the Springleik project repository on GitHub \cite{GitHubRepo}.}
\label{toyModel}
\end{figure}
...
In addition to the interactive command lines, the target and host programs also accept script files containing commands and arguments appropriate for each. Script files read by the target program contain target commands which interact with (simulated) hardware, while script files read by the host program contain host commands which interact with the AST interpreter.

\clearpage

\begin{thebibliography}{99}
\bibitem{chatGPTIntro}
Section \ref{Introduction} written by ChatGPT-5.5 on 18 July 2026, based on conversations with the author.
\bibitem{Alexander1}
...
\bibitem{ControlSys}
Feedback Control Systems, 3rd edn. (Phillips \& Harbor 1996)
\bibitem{Stroustrup}
The C++ Programming Language, 2nd edn. (Stroustrup 1991)
\bibitem{Knuth}
Papers suggested by Google AI
\bibitem{GitHubRepo}
...
Baxter \& Mehlich 2000.
\bibitem{JEReich}
Reich, Krogh, \& Baxter 1996
\bibitem{asymmetricRules}
Description of asymmetric tree comparison published previously at \\ \url{https://github.com/springleik/PompTree}
\end{thebibliography}
\section{Biography}
