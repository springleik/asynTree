...
Motor Control: Iterate 3 times.
  Track #1:
    Move motor 0 to target 135 with increment 15 deg.
    Move motor 1 to target 135 with increment 15 deg.
    Wait for motor 0 target reached.
    Wait for key i input.
    Wait for motor 1 target reached.
    Move motor 0 to target 270 with increment 3 deg.
    Wait for motor 0 target reached.
    Wait for key j input.
    Move motor 0 to target 135 with increment 15 deg.
    Move motor 1 to target 270 with increment 3 deg.
    Wait for motor 1 target reached.
    Wait track 1: Wait for 1.0 seconds.
    Move motor 1 to target 135 with increment 15 deg.
\end{lstlisting}
\end{minipage}
\caption{Pseudocode generatated automatically by a Python script implementing domain-specific knowledge of the toy motor program's compsite tree structure.}
\label{pseudoCode}
\end{figure}
...
\end{lstlisting}
\end{minipage}
\caption{Hand-typed comment text corresponding to the automatically generated output in figure \ref{pseudoCode}.}
\label{commentText}
\end{figure}

The toy motor subproject includes a Python script which renders a JSON serialization of the control program as pseudocode. The script implements domain-specific knowledge which lets it emit pseudocode representing the design intent of each node. A snippet of its output appears in figure \ref{pseudoCode}. The script was surprisingly easy to implement, and worked on the first try. We propose that the reason for this is that pseudocode has a precise one-to-one mapping to composite tree structures. A corresponding snippet of pseudocode is shown in figure \ref{commentText} which is taken from hand-typed comment text written when the project began. They are not identical, but are easily compared when viewed together.
...
\begin{wrapfigure}{l}{32mm}
    \includegraphics[width=1.25in,height=1.5625in,clip,keepaspectratio]{Portrait1}
\end{wrapfigure}
\textbf{Mark Williamsen} received a B.S. degree in physics and a Ph.D. degree in physics from the University of Wisconsin at Milwaukee in 2003 and 2010 respectively. He has previously worked at Rockwell Automation in Milwaukee and Quantum Design in San Diego, USA. He is currently employed at Coesia FlexLink in Gothenburg, Sweden working on embedded firmware applications for the industrial internet-of-things (IIoT).
